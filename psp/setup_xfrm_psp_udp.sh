...
if [ $FULL == 0 ]; then
	...
	if [ $OFFLOAD == 2 ]; then
		...
		OFFLOAD_OUT_REMOTE="$FULL_OFFLOAD_TYPE dev $REMOTE_IFNAME dir out address $LOCAL_MAC"
		OFFLOAD_IN_REMOTE="$FULL_OFFLOAD_TYPE dev $REMOTE_IFNAME dir in address $LOCAL_MAC"
	elif [ $OFFLOAD == 1 ]; then
		OFFLOAD_OUT="$FULL_OFFLOAD_TYPE dev $IFNAME dir out $DSTADDR"
		OFFLOAD_IN="$FULL_OFFLOAD_TYPE dev $IFNAME dir in $DSTADDR"
	fi
...
# KEY_IN=0x8ec46e5fbd38688108ce490ed96697e82acedc78429e31a88f536f87f6ae4ff4bb772f7a
# KEY_OUT=0x06cb288f8d58b27a3347ab053adc473cbbbe5c50d142c5abf9b50d449427cb9c322ed998
#Key produced by REMOTE
CHANGES=6bb7f0e5c282ba01f3bc2b59d0cdf446
KEY_IN=0x${CHANGES}00000000
#Key produced by LOCAL
CHAREMS=002bbdfb413771b0d85918b54444ffd1
#KEY_OUT=0x3b7a1430395fc0c9367758212b6a7e8700000000
KEY_OUT=0x${CHAREMS}00000000

DPORT=4500
DPORT=1000
SPORT=38072
SPORT=4500
UDPEL="encap espinudp $SPORT $DPORT 0.0.0.0"
UDPER="encap espinudp $DPORT $SPORT 0.0.0.0"
...
sudo ip xfrm state add src $SRC dst $DST proto esp spi $ID_IN reqid $ID_IN mode $ESP_MODE aead 'rfc4106(gcm(aes))' $KEY_IN $GCMSIZE $OFFLOAD_OUT sel src $LOCALNET dst $REMOTENET $UPSPEC $UDPEL
#sudo ip xfrm state add src $DST dst $SRC proto esp spi $ID_OUT reqid $ID_OUT mode $ESP_MODE aead 'rfc4106(gcm(aes))' $KEY_OUT $GCMSIZE $OFFLOAD_IN sel src $REMOTENET dst $LOCALNET $UPSPEC $UDPER
...
ssh root@$REMOTE /bin/bash << EOF
	...
	sudo ip xfrm state add src $SRC dst $DST proto esp spi $ID_IN reqid $ID_IN mode $ESP_MODE aead 'rfc4106(gcm(aes))' $KEY_IN $GCMSIZE $OFFLOAD_IN_REMOTE sel src $LOCALNET dst $REMOTENET $UPSPEC $UDPEL
	sudo ip xfrm state add src $DST dst $SRC proto esp spi $ID_OUT reqid $ID_OUT mode $ESP_MODE aead 'rfc4106(gcm(aes))' $KEY_OUT $GCMSIZE $OFFLOAD_OUT_REMOTE sel src $REMOTENET dst $LOCALNET $UPSPEC $UDPER
EOF
