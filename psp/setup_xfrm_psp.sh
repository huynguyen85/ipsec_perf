...
CHANGES=87888ea7945c039679ff03630263c0ec
KEY_REMOTE=0x${CHANGES}00000000
#Key produced by LOCAL
CHAREMS=9b2e6f2fa8551704b2dda680db08a3a0
#KEY_OUT=0x3b7a1430395fc0c9367758212b6a7e8700000000
KEY_LOCAL=0x${CHAREMS}00000000
...
sudo ip xfrm state add src $SRC dst $DST proto esp spi $ID_IN reqid $ID_IN mode $ESP_MODE aead 'rfc4106(gcm(aes))' $KEY_REMOTE $GCMSIZE $OFFLOAD_OUT sel src $LOCALNET dst $REMOTENET $UPSPEC
sudo ip xfrm state add src $DST dst $SRC proto esp spi $ID_OUT reqid $ID_OUT mode $ESP_MODE aead 'rfc4106(gcm(aes))' $KEY_LOCAL $GCMSIZE $OFFLOAD_IN sel src $REMOTENET dst $LOCALNET $UPSPEC
...
ssh root@$REMOTE /bin/bash << EOF
	...
	sudo ip xfrm state add src $SRC dst $DST proto esp spi $ID_IN reqid $ID_IN mode $ESP_MODE aead 'rfc4106(gcm(aes))' $KEY_REMOTE $GCMSIZE $OFFLOAD_IN_REMOTE sel src $LOCALNET dst $REMOTENET $UPSPEC
	sudo ip xfrm state add src $DST dst $SRC proto esp spi $ID_OUT reqid $ID_OUT mode $ESP_MODE aead 'rfc4106(gcm(aes))' $KEY_LOCAL $GCMSIZE $OFFLOAD_OUT_REMOTE sel src $REMOTENET dst $LOCALNET $UPSPEC
EOF
